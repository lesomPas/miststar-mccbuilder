...
class MCDParserV2(MCDParserV1):
    def generate_mcd(self, doc: str) -> MCD:
        ...
        chain_count = 0
        cur_chain = self._generate_prefix_chain(cursor)
    ...
    def _generate_prefix_chain(self, cursor: DocumentCursor) -> str | ChainState:
        chain_item = []
        ...
        while cursor.current is not None:
            ...
            match cur.kind:
                case DocumentTokenKind.ChainLabel:
                    self.chains.append(MCDChain(name="root_comments", items=chain_item))
                    cursor.bump()
                    return cur.data
